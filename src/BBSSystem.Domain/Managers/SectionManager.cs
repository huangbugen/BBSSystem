...
namespace BBSSystem.Domain.Managers
{
    public class SectionManager : DomainService
    {
        ...
        public async Task<List<Section>> GetSectionsAsync(List<string> areaIds, int skip = 0, int take = 30)
        {
        ...
        public async Task<(Section section, List<string> lords)> GetSimpleSectionsAsync(string sectionId)
        {
            ...
            var lords = sectionLords.Select(m => m.UserName).ToList();

            return (section, lords);
        }
    }
}